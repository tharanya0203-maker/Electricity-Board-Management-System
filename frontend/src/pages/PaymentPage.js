...
const PaymentPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('pay-bill'); // 'pay-bill' or 'payment-history'
  ...
  if (loading) {
    return <div className="container"><div className="card">Loading payment information...</div></div>;
  }
  ...
  return (
    <div className="container">
      ...
      <div className="tabs">
        <button 
          className={`tab-button ${activeTab === 'pay-bill' ? 'active' : ''}`}
                ...
                <tbody>
                  {unpaidBills.map(bill => (
                    <tr key={bill._id}>
                      ...
                      <td>{bill.billAmount + (bill.dueAmount || 0)}</td>
                      <td>
                        <button 
              ...
              <table className="table">
                ...
                <tbody>
                  {payments.map(payment => (
                    <tr key={payment._id}>
                      <td>{payment.billId.month}</td>
                      <td>{payment.billId.year}</td>
